...
class Campaigns extends AbstractDao {
	...
	public function getReviewers( $id = null ) {
		if ( $id == null ) {
			...
			$sql = self::concat(
				...
				'FROM users u, campaign_users cu WHERE u.id = cu.user_id AND cu.campaign_id = ?'
			);
	...
	*/
	 ...
	 */
	private function addReviewers( $id, $reviewers ) {
	 ...
	 */
	private function removeReviewers( $id, $reviewers ) {
		foreach ( $reviewers as $r ) {
			$sql = self::concat(
