	...
	protected function configureRoutes() {
		...
		$slim->group( '/campaign/:campaign/proposals/',
			...
			function () use ( $slim, $middleware ) {
				...
				$slim->get( ':id/edit', function ( $id, $campaign ) use ( $slim ) {
					$page = new Controllers\Proposals\Edit( $slim );
					$page->setDao( $slim->proposalsDao );
					$page( $id, $campaign );
				} )->name( 'proposals_edit' );

				$slim->post( ':id/edit/post', function ( $id, $campaign ) use ( $slim ) {
					$page = new Controllers\Proposals\Edit( $slim );
					$page->setDao( $slim->proposalsDao );
					$page( $id, $campaign );
				} )->name( 'proposals_edit_post' );

				$slim->post( ':id/review', function ( $id, $campaign ) use ( $slim ) {
				...
				$slim->get( ':id', function ( $id, $campaign ) use ( $slim ) {
